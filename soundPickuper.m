...
elseif fs==22050
    if bpm==100
        neko1 = audioread('neko1_bpm100_22050Hz.wav');
        neko2 = audioread('neko2_bpm100_22050Hz.wav');
        neko3 = audioread('neko3_bpm100_22050Hz.wav');
    elseif bpm==120
        neko1 = audioread('neko1_bpm120_22050Hz.wav');
        neko2 = audioread('neko2_bpm120_22050Hz.wav');
        neko3 = audioread('neko3_bpm120_22050Hz.wav');
    end
end
neko = [];
neko(1,:) = neko1;
...
audio_random_fade05_neko = [];
audio_random_fade08_neko = [];
audio_random_fade10_neko = [];
audio_fade05_neko = [];
audio_fade08_neko = [];
audio_fade10_neko = [];

neko_2bar = length(neko(1,:))-aoSampleLength;
for n=1:3
    for i=1:length(audio_sample(:,1))
        audio_random_fade05_neko(i,:) = [neko(n,1:neko_2bar),...
            audio_random_fade05(i,:) + neko(n,(neko_2bar+1):end)];
        audio_random_fade08_neko(i,:) = [neko(n,1:neko_2bar),...
            audio_random_fade08(i,:) + neko(n,(neko_2bar+1):end)];
        audio_random_fade10_neko(i,:) = [neko(n,1:neko_2bar),...
            audio_random_fade10(i,:) + neko(n,(neko_2bar+1):end)];
        
        audio_fade05_neko(i,:) = [neko(n,1:neko_2bar),...
            audio_fade05(i,:) + neko(n,(neko_2bar+1):end)];
        audio_fade08_neko(i,:) = [neko(n,1:neko_2bar),...
            audio_fade08(i,:) + neko(n,(neko_2bar+1):end)];
        audio_fade10_neko(i,:) = [neko(n,1:neko_2bar),...
            audio_fade10(i,:) + neko(n,(neko_2bar+1):end)];
    end

% audio_fade05 = audio_fade05_neko;
% audio_fade08 = audio_fade08_neko;
% audio_fade10 = audio_fade10_neko;
% audio_random_fade05 = audio_random_fade05_neko;
% audio_random_fade08 = audio_random_fade08_neko;
% audio_random_fade10 = audio_random_fade10_neko;
...
if is_getaudio == 1
    ...
    for i=1:length(audio_sample(:,1))
        if SNratio(i) >=0.2
            pass = ['\Users\Shunji\Music\RandomPickup\','bpm',num2str(bpm),'\',...
               fname_withoutWAV,'\neko',num2str(n),'\'];
            %pass = ['\Users\Shunji\Music\RandomPickup\neko',num2str(n),'\'];
            ...
            wfname6 = [pass,'random_fade08_scene',num2str(i),'_',...
                ...
                '_section',num2str(str_random(i).table.s_start(1)),...
                'to',num2str(str_random(i).table.s_end(1)),'.wav'];
            audiowrite(wfname1,audio_fade10_neko(i,:),fs);
            audiowrite(wfname2,audio_fade05_neko(i,:),fs);
            audiowrite(wfname3,audio_random_fade10_neko(i,:),fs);
            audiowrite(wfname4,audio_random_fade05_neko(i,:),fs);
            audiowrite(wfname5,audio_fade08_neko(i,:),fs);
            audiowrite(wfname6,audio_random_fade08_neko(i,:),fs);
        end
    end
