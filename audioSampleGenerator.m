...
%     bars    : 小節数
%     beat_numer : 1小節あたりの拍数
%     beat_demon : 単位音符
%     is_plot : ピーク検出結果をプロットするかどうか (0:off else:on)

% audio_outputの秒数を取得
aoBeats = bars*beat_numer;
aoSecLength = (60/bpm)*aoBeats;
aoSampleLength = aoSecLength * fs;
bpsample = round(fs*60/bpm*(beat_denom/beat_numer));
...
% エンベロープからピークを取得
thr = mean(env);
[~,locs_peak] = findpeaks(env,'MinPeakDistance',bpsample,'MinPeakHeight',thr);
...
for i=1:length(locs_peak)      
    ...
    [~,locs_rev] = findpeaks(env_rev_inverted,'NPeaks',1,'MinPeakHeight',thr);
...
if is_plot ~= 0
    ...
    hold on;
    plot(t,env);
    plot(locs_peak,env(locs_peak),'rv','MarkerFaceColor','r');
    plot(locs_valley,env(locs_valley),'rs','MarkerFaceColor','b');
    hold off;
    xlim([0 length(env)]);
    title('Audio Peak Picking');
end
